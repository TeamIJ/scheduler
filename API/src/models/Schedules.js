...
module.exports = {
    ...
    findBySearchFilters(req, res, registry, student, professor, somenteDaSemana, status) {
        ...
        `SELECT A.ID_AGENDA AS id, AL.NOME AS aluno, P.NOME AS professor, C.NOME_CURSO AS curso, M.NOME_MODULO AS modulo,
         ...
         INNER JOIN CURSOS C ON C.ID_CURSO = A.ID_CURSO AND C.ID_CURSO = A.ID_CURSO
         INNER JOIN MODULOS M ON M.ID_CURSO = C.ID_CURSO AND M.ID_MODULO = A.ID_MODULO
         INNER JOIN PROFESSORES P ON A.ID_PROF = P.ID_PROF AND P.ID_PROF = A.ID_PROF
         INNER JOIN PROFESSORES_CURSOS P_C ON P_C.ID_PROF = P.ID_PROF AND P_C.ID_PROF = A.ID_PROF AND P_C.ID_CURSO = A.ID_CURSO
         WHERE 1=1`
